...
model_0 = simple_rnn_model(input_dim=13)#161) # change to 13 if you would like to use MFCC features
...
train_model(input_to_softmax=model_0, 
            ...
            spectrogram=False)#True) # change to False if you would like to use MFCC features
...
model_1 = rnn_model(input_dim=13,#161, # change to 13 if you would like to use MFCC features
                    units=200,
                    activation='relu')
...
model_2 = cnn_rnn_model(input_dim=13,#161, # change to 13 if you would like to use MFCC features
                        filters=200,
                        kernel_size=11, 
                        conv_stride=2,
...
model_3 = deep_rnn_model(input_dim=13,#161, # change to 13 if you would like to use MFCC features
                         units=200,
                         recur_layers=2) 
...
model_4 = bidirectional_rnn_model(input_dim=13,#161, # change to 13 if you would like to use MFCC features
                                  units=200)
...
# 
# __Answer:__
# Model 1: Simple RNN solution. The training and validation loss are large and do not decrease as epoch gets larger.
# 
# Model 3: Deep RNN solution. The training and validation loss are smaller than the simple RNN solution. The losses decrease sharply as epoch gets larger.
# 
# Model 4: Bidirectional RNN solution. The training and validation loss are smaller than the simple RNN solution. The losses keep at low level and decrease as epoch gets larger.
# 
# Model 3 and Model 4 performs best. 
# 
# Model 3: Deep RNN model training loss converges fast with larger epoch rounds.
# 
# Model 4:Bidirectional RNN model keeps information bidirectionally which  helps it keep a low training loss
...
# specify the model
model_end = final_model(input_dim=13, output_dim=29)
...
# 
# __Answer:__ The final model includes two bidirectional layers with dropout=0.2 is a technique to avoid overfitting
...
get_predictions(index=0, 
                partition='train',
                input_to_softmax=final_model(input_dim=13, output_dim=29), 
